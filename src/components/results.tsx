...
function StatCard({ label, value, tone = "ink", sub }: { label: string; value: number; tone?: "ink" | "blue" | "brass" | "moss" | "rust"; sub?: string }) {
  ...
  return (
    <div className={`border border-ink-900/12 border-t-4 bg-white/85 px-4 py-3.5 shadow-[0_1px_0_rgba(14,24,35,0.05)] transition-transform duration-200 hover:-translate-y-0.5 ${tones[tone]}`}>
      <div className="font-mono text-2xl font-bold leading-none tracking-tight sm:text-[27px]">
        <CountUp value={value} />
      </div>
      <div className="mt-1.5 text-[11px] font-semibold uppercase tracking-[0.08em] text-ink-400">{label}</div>
      {sub && <div className="mt-0.5 font-mono text-[10.5px] text-ink-300">{sub}</div>}
    </div>
  );
}

export function Results({ res, xlsxUrl, csvUrl }: { res: VorResult; xlsxUrl: string | null; csvUrl: string | null }) {
    ...
    <section id="result" className="space-y-4">
          ...
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div>
              <div className="font-mono text-[10.5px] uppercase tracking-[0.22em] text-brass-500">
                Лист «ВОР» · ВОР.xlsx · {res.generatedAt ? res.generatedAt.toLocaleString("ru-RU") : ""}
              </div>
              ...
              <div className="mt-1 font-mono text-[11.5px] text-ink-300">
                Навигатор: Л2 = <span className="text-brass-400">{res.l2}</span> · Л3 = <span className="text-brass-400">{res.l3}</span>
              </div>
            </div>
        ...
        <Reveal delay={40}>
          <div className="space-y-1.5">
            {res.warnings.map((w, i) => (
              <div key={i} className="flex items-start gap-2.5 border border-brass-500/50 bg-brass-100/60 px-4 py-2.5 text-[12.5px] text-[#6d4e05]">
      ...
      <Reveal delay={60}>
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5">
          ...
          <StatCard label="Подобрано Код ТМЦ" value={s.tmcFound} tone="moss" sub={`точно ${s.tmcExact} · группа ${s.tmcGroup} · аналог ${s.tmcAnalog}`} />
          ...
          <div className="flex flex-wrap items-center gap-2 border-b-2 border-ink-900 px-4 py-2.5">
            <button
              ...
              className={`border px-3 py-1 font-mono text-[11.5px] font-bold uppercase tracking-wider transition-colors ${
                tab === "vor" ? "border-ink-900 bg-ink-900 text-brass-400" : "border-ink-900/25 text-ink-400 hover:border-ink-900 hover:text-ink-800"
              }`}
            >
              Предпросмотр ВОР
            </button>
            <button
              onClick={() => setTab("nf")}
              className={`border px-3 py-1 font-mono text-[11.5px] font-bold uppercase tracking-wider transition-colors ${
                tab === "nf" ? "border-rust-600 bg-rust-600 text-white" : "border-ink-900/25 text-ink-400 hover:border-rust-600 hover:text-rust-600"
              }`}
            >
              ...
              <table className="w-full min-w-[980px] text-left text-[12px]">
                <thead className="sticky top-0 z-10">
                  <tr className="bg-ink-800 text-paper-50">
                    {["№ п/п", "Система", "Строка", "Этаж", "Наименование", "ЕИ", "Кол-во", "Код КЕР", "Код ТМЦ", "Расход", "ТА"].map((h) => (
                ...
                <tbody>
                  {preview.map((r, i) => (
                    <tr key={i} className={`row-in border-t border-ink-900/8 align-top transition-colors hover:bg-brass-100/45 ${r.ta === "Заголовок" ? "bg-ink-900/[0.045] font-semibold" : i % 2 ? "bg-ink-900/[0.02]" : ""}`}>
                      <td className="px-3 py-1.5 font-mono text-[11.5px] font-bold text-blueprint-700">'{r.npp}</td>
                      ...
                      <td className="max-w-[420px] px-3 py-1.5 leading-snug text-ink-800">{r.name}</td>
                      <td className="whitespace-nowrap px-3 py-1.5 text-ink-400">{r.unit || "—"}</td>
                      <td className="whitespace-nowrap px-3 py-1.5 text-right font-mono text-[11.5px] text-ink-800">{fmtQty(r.qty)}</td>
                      <td className="px-3 py-1.5 text-center font-mono text-[11.5px] font-bold text-blueprint-700">{r.kerId ?? ""}</td>
                      <td className="px-3 py-1.5 text-center font-mono text-[11.5px] font-bold text-moss-600">{r.tmcId ?? ""}</td>
                      <td className="whitespace-nowrap px-3 py-1.5 text-right font-mono text-[11.5px] text-ink-800">{fmtQty(r.rashod)}</td>
                      <td className="whitespace-nowrap px-3 py-1.5"><TaChip ta={r.ta} /></td>
                    </tr>
                  ))}
                </tbody>
              </table>
                    ...
                    res.notFound.map((r, i) => (
                      <tr key={i} className="border-t border-ink-900/8 transition-colors odd:bg-ink-900/[0.02] hover:bg-rust-100/50">
                        ...
                        <td className="whitespace-nowrap px-3 py-1.5">
                          <span className={`border px-1.5 py-0.5 font-mono text-[10.5px] font-bold ${r.what === "Код КЕР" ? "border-brass-500/50 bg-brass-100 text-[#8a6206]" : "border-moss-500/50 bg-moss-100 text-moss-600"}`}>
                            {r.what}
                          </span>
                        </td>
      ...
      <Reveal delay={160}>
        <div className="flex flex-wrap items-center gap-3 border border-ink-900/12 bg-ink-900 px-5 py-4">
          ...
          {xlsxUrl && (
            <a href={xlsxUrl} download="ВОР.xlsx" className="group inline-flex items-center gap-2 bg-brass-500 px-5 py-2.5 font-display text-[12px] font-bold uppercase tracking-[0.1em] text-ink-950 transition-all hover:-translate-y-0.5 hover:bg-brass-400 hover:shadow-[0_6px_18px_rgba(240,168,28,0.35)] active:translate-y-0">
              <IconDownload className="h-4 w-4 transition-transform group-hover:translate-y-0.5" />
              Скачать ВОР.xlsx
            </a>
          )}
          {csvUrl && (
            <a href={csvUrl} download="ВОР_с_ТА.csv" className="inline-flex items-center gap-2 border border-ink-300/40 px-4 py-2.5 font-mono text-[11.5px] font-semibold uppercase tracking-wider text-ink-200 transition-colors hover:border-brass-500 hover:text-brass-400">
              ВОР_с_ТА.csv
            </a>
          )}
        </div>
