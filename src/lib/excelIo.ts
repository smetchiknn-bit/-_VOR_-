...
function makeWorkbook(res: VorResult, prompt: PromptState): XLSX.WorkBook {
  const wb = XLSX.utils.book_new();
  
  // Лист "ВОР"
  const vorData = vorSheetRows(res);
  const wsVor = XLSX.utils.json_to_sheet(vorData);
  ...
  const wsStat = XLSX.utils.json_to_sheet(statData);
  wsStat["!cols"] = [{ wch: 44 }, { wch: 28 }];
  XLSX.utils.book_append_sheet(wb, wsStat, "Статистика");
  
  // Лист "Не найдено"
  const nfData = notFoundSheetRows(res);
  const wsNf = XLSX.utils.json_to_sheet(nfData);
  wsNf["!cols"] = [
    { wch: 12 },  // Система
    { wch: 10 },  // Строка
    { wch: 60 },  // Наименование
    { wch: 8 },   // Кол-во
    { wch: 14 },  // Что не найдено
    { wch: 44 }   // Причина
  ];
