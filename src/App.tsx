          ...
          <div>
            <div className="mb-1.5 flex items-baseline justify-between">
              ...
              <span className="font-mono text-[10.5px] text-ink-300">
                {(draft.length || 0).toLocaleString("ru-RU")} симв.
              </span>
            </div>
            <textarea
              value={draft}
...
function AppInner() {
  ...
  const run = async () => {
    ...
    try {
      // Отладка: проверяем загруженные файлы
      console.log("Спецификация:", files.spec?.rows.length, "строк");
      console.log("База КЕР:", files.ker?.rows.length, "строк");
      console.log("База ТМЦ:", files.tmc?.rows.length, "строк");
      ...
      console.log("Результат:", res.stats);
              ...
              {stage === "done" && result && (
                <div className="space-y-2">
                  <div className="flex items-start gap-2.5 border border-moss-500/50 bg-moss-500/10 px-3.5 py-3 text-[12px] leading-snug text-moss-500">
                    ...
                    ВОР готов: {(result.stats.vorTotal || 0).toLocaleString("ru-RU")} строк · КЕР —{" "}
                    {result.stats.kerFound || 0} · ТМЦ — {result.stats.tmcFound || 0} · «Не найдено» —{" "}
                    {result.stats.notFoundRows || 0}.
                  </div>
                  {downloads && (
              ...
              <div className="flex items-center gap-3 border-l border-ink-900/15 px-6 py-5 lg:border-l-0">
                ...
                <div>
                  ...
                  <div className="font-mono text-[10.5px] text-ink-400">
                    {clock ? clock.toLocaleDateString("ru-RU") : ""} {clock ? clock.toLocaleTimeString("ru-RU") : ""}
                  </div>
                </div>
              </div>
            </div>
